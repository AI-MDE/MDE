 ...
 * 1) Load viewer/project configuration.
 * 2) Build a document tree from catalog + scanned folders.
 * 3) Serve static viewer assets and JSON API endpoints.
 * 4) Read/edit project documents in allowed folders.
 * 5) Run local tooling commands and capture simple logs.
 *
 * This file is still a single-process Node HTTP server, but the sections below
 * are grouped so it is easier to follow the startup flow and the runtime flow.
 */

const http = require("http");
const fs = require("fs");
const path = require("path");
const { ConfigurationManager } = require(
  path.join(__dirname, "..", "tools", "lib", "config-manager"),
);
...
if (!fs.existsSync(DOCS_ROOT)) {
  ...
  console.error(
    `[viewer] Use: node mde/web/app.js  OR  node mde/web/viewer.js --root=<path>`,
  );
  process.exit(1);
}
...
// Base config shape used when config files are absent or incomplete.
...
try {
  ...
  console.warn(
    "Warning: configuration.json not found, using built-in defaults.",
  );
}
...
try {
  ...
  if (appStructure && typeof appStructure === "object") {
    if (cfg.catalog === undefined && Array.isArray(appStructure.catalog))
      cfg.catalog = appStructure.catalog;
    if (!cfg.patterns || Object.keys(cfg.patterns).length === 0)
      cfg.patterns = appStructure.patterns || {};
  }
} catch {
  /* appStructure.json is optional */
}

// Legacy config keys are supported because older projects may still use MDT-path.
const MDE_PATH_REL =
  cfg["mde-path"] || cfg["MDT-path"] || cfg["mdt-path"] || null;
...
function resolveConfigValue(obj, keyPath) {
  ...
  if (keyPath.startsWith("details.locations.")) {
    ...
    return ((obj &&
      obj.details &&
      (obj.details.locations || obj.details.paths)) ||
      obj.paths ||
      {})[pathKey];
  }
  ...
  if (keyPath.startsWith("details.folders.")) {
    ...
    return ((obj &&
      obj.details &&
      (obj.details.folders || obj.details["project-folders"])) ||
      obj["project-folders"] ||
      {})[folderKey];
  }
...
// Template paths keyed by docType. Loaded later from configuration.
...
function loadCommandRegistry() {
  const folderRel =
    resolveConfigValue(cfg, "details.paths.commands-dir") ||
    resolveConfigValue(cfg, "paths.commands-dir") ||
    (MDE_PATH_REL && `${MDE_PATH_REL}/ai-instructions/commands`);
  if (!folderRel) {
    console.warn(
      "[viewer] commands-dir not configured; command registry will be empty.",
    );
    return [];
  }
...
// -----------------------------------------------------------------------------
// Logging and process-level error handling
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Resolved project paths and patterns
// -----------------------------------------------------------------------------

const P =
  (cfg.details && (cfg.details.locations || cfg.details.paths)) ||
  cfg.paths ||
  {};
 ...
 */
// -----------------------------------------------------------------------------
// Catalog compilation: config catalog -> normalized viewer tree
// -----------------------------------------------------------------------------
  ...
  switch (kind) {
    ...
    case "entity":
      return {
        ...
        label: entLabel(filename),
        file: rel,
        docType: "entity",
      };
    case "module":
      return {
        id: rel.replace(/\.json$/i, ""),
        label: modLabel(filename),
        file: rel,
        docType: "module",
      };
    default:
...
function compileGroup(group) {
  ...
  let docs = [];
  if (group.scan) {
    const dir = group.scan.dir || resolveConfigValue(cfg, group.scan.dirRef);
    if (!dir) {
      logWarn(
        `Group "${group.id || group.label}" has a scan with no dir; skipping.`,
      );
      return null;
    }
    const patternSource =
    ...
    const files = group.scan.recursive
      ? scanDirRecursive(dir, pattern)
      : scanDir(dir, pattern);
    docs = files.map((file) => {
      const relFile = group.scan.recursive
        ? normalizeRel(file)
        : `${normalizeRel(dir)}/${file}`;
...
function buildPhases() {
  ...
  if (!catalog) {
    logWarn(
      "No catalog found in configuration. Run: node mde/scripts/generateAppStructure.js",
    );
    return [];
  }
  return catalog
    ...
    .map((phase) => {
      ...
      if (docs.length === 0 && groups.length === 0 && !phase.includeIfEmpty)
        return null;
      const compiled = {
...
console.log(
  `[viewer] Loaded configuration with ${PHASES.length} phases, ${commandRegistry.length} commands, and ${Object.keys(cfg.patterns || {}).length} patterns.`,
);
...
function semanticPhaseFromCatalog(phase) {
  ...
  if (txt.includes("project workspace") || txt.includes("project initiation"))
    return "project_initiation";
  if (txt.includes("entity")) return "system_design";
  if (
    txt.includes("test") ||
    txt.includes("traceability") ||
...
function loadCommandBindings() {
  ...
  (PHASES || []).forEach((phase) => {
    ...
    const derived = filterAvailable(
      commandNamesForPhase(commandRegistry, semantic),
    );
    phases[phase.id] = mergeLists(
      derived,
      filterAvailable(fallback.phases && fallback.phases[phase.id]),
    );
  });
  ...
  Object.entries(cfg.doctypes || {}).forEach(([docType, entry]) => {
    ...
    docTypes[docType] = mergeLists(
      exceptionOnly,
      filterAvailable(fallback.docTypes && fallback.docTypes[docType]),
    );

    const ids = Array.isArray(entry && entry.ids) ? entry.ids : [];
    ids.forEach((id) => {
      docIds[id] = mergeLists(docIds[id], docTypes[docType]);
      docIds[id] = mergeLists(
        docIds[id],
        filterAvailable(fallback.docIds && fallback.docIds[id]),
      );
    });
  });
...
// -----------------------------------------------------------------------------
// In-memory document indexes and template configuration
// -----------------------------------------------------------------------------
...
try {
  ...
  doctypeTemplates = Object.entries(docTypeEntries).reduce(
    (acc, [docType, config]) => {
 ...
 */
// -----------------------------------------------------------------------------
// File scanning and lightweight templating
// -----------------------------------------------------------------------------

function scanDir(subdir, pattern) {
  try {
    return fs
      .readdirSync(path.join(DOCS_ROOT, subdir), { withFileTypes: true })
...
function buildTemplateData(parsed, relPath) {
  ...
  if (data && data.phase_rules && typeof data.phase_rules === "object") {
    const phaseRules =
      data.phase_rules && typeof data.phase_rules === "object"
        ? data.phase_rules
        : {};

    data.phase_rules_list = Object.entries(phaseRules).map(
      ([phase, rules], index, all) => ({
        phase,
        phase_index: index + 1,
        next_phase: all[index + 1] ? all[index + 1][0] : "",
        allowed_commands: Array.isArray(rules && rules.allowed_commands)
          ? rules.allowed_commands
          : [],
        entry_conditions: Array.isArray(rules && rules.entry_conditions)
          ? rules.entry_conditions
          : [],
        exit_conditions: Array.isArray(rules && rules.exit_conditions)
...
function renderTemplate(template, data, _depth = 0) {
  if (_depth > 20) {
    logWarn("renderTemplate: max depth exceeded — possible infinite loop");
    return "";
  }
  let out = template;
 ...
 * Main request router.
 *
 * Routes are kept inline so the server remains easy to run as a single file,
 * but they are grouped by concern: static assets, metadata APIs, command APIs,
 * document APIs, and creation APIs.
 */
const server = http.createServer((req, res) => {
  ...
  try {
    ...
    if (url.pathname === "/" || url.pathname === "/viewer.html") {
      ...
      // Read-only metadata used by the client UI.
      ...
      // Prompt generation and command logging helpers.
      ...
      req.on("end", () => {
        try {
          ...
          if (!TOOLS_ROOT) {
            res.writeHead(500);
            res.end(
              "tools-dir not configured; set mde-path in configuration.json",
            );
            return;
          }
          const cfgLocal = readConfig();
          const cfgPaths =
            (cfgLocal.details &&
              (cfgLocal.details.locations || cfgLocal.details.paths)) ||
            cfgLocal.paths ||
            {};
          ...
          if (command === "validate_requirements") {
            ...
            const args = [
              `--config=${path.join(DOCS_ROOT, "configuration.json")}`,
            ];
            runTool(tool, args, (result) => json(res, result), {
              command,
              label: cmdLabel,
            });
            return;
          ...
          // Try both underscore and kebab-case tool names so commands remain forgiving.
          ...
          if (toolPath) {
            ...
            runTool(toolPath, args, (result) => json(res, result), {
              command,
              label: cmdLabel,
            });
            return;
        ...
        const entities = files
          .map((f) => {
            try {
              return JSON.parse(
                stripBom(
                  fs.readFileSync(path.join(DOCS_ROOT, entScan.dir, f), "utf8"),
                ),
              );
            } catch {
              return null;
      ...
      // Only allow reads/writes inside known safe roots under the current docs project.
      ...
      if (req.method === "GET") {
        ...
        try {
          ...
          /* file not found — let readFile produce the error */
        }
        ...
        fs.readFile(resolvedAbs, "utf8", (err, data) => {
          ...
          try {
            ...
            if (ext === "json") {
              ...
              // raw=1 lets the client bypass server-side template rendering for debugging.
            ...
            json(res, {
              content: data,
              ext: path.extname(resolvedAbs).slice(1),
            });
          } catch (e) {
      ...
      // Scaffold brand-new documents using simple built-in templates.
      ...
      req.on("end", () => {
        try {
          ...
          if (type === "use-case") {
            if (!ucDir) {
              res.writeHead(404);
              res.end("No use-case directory configured in catalog");
              return;
            }
            ...
            if (!entDir) {
              res.writeHead(404);
              res.end("No entity directory configured in catalog");
              return;
            }
 ...
 */
// -----------------------------------------------------------------------------
// Logging helpers for viewer actions and command execution
// -----------------------------------------------------------------------------
 ...
 */
// -----------------------------------------------------------------------------
// Prompt generation and tool execution
// -----------------------------------------------------------------------------
...
function appendCommandLog(entry) {
  try {
    ...
    if (fs.existsSync(COMMAND_LOG)) {
      try {
        log = JSON.parse(fs.readFileSync(COMMAND_LOG, "utf8"));
      } catch {
        /* start fresh */
      }
    }
...
function runTool(cmd, args, cb, meta = {}) {
  ...
  // process.execPath ensures the same Node runtime launches the child script.

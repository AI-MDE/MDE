...
class DocServer {
    ...
    resolveFileItem(item) {
        ...
        if (item.includeIfExists && !fs.existsSync(path.resolve(this.docsRoot, file)))
            return null;
        return {
            id: item.id,
    ...
    resolveScanItem(item) {
        ...
        if (!relFiles.length)
            return null;
        if (groupBy === 'phase') {
            const groupByOrder = scan.groupByOrder ?? [];
            const byPhase = new Map();
            for (const relFile of relFiles) {
                let phase = 'other';
                try {
                    const abs = path.resolve(this.docsRoot, relFile);
                    const obj = JSON.parse(fs.readFileSync(abs, 'utf8').replace(/^\uFEFF/, ''));
            ...
            const subGroups = orderedKeys.map(phase => ({
    ...
    async takeScreenshot(targetUrl) {
        const browser = await this.getBrowser();
        const page = await browser.newPage();
        await page.setViewportSize({ width: 1440, height: 900 });
        try {
            // Derive base URL (origin) from targetUrl
            const parsed = new URL(targetUrl);
            const loginUrl = `${parsed.origin}/login`;
            // Log in first (mock auth accepts any credentials)
    ...
    async handle(req, res) {
        ...
        try {
            ...
            if (url.pathname === '/erd-demo.html') {
                return this.serveStatic(res, path.join(__dirname, 'erd-demo.html'), 'text/html');
            }
            if (url.pathname === '/erd-manager.js') {
                return this.serveStatic(res, path.join(__dirname, 'erd-manager.js'), 'text/javascript');
            }
            if (url.pathname === '/erd-demo.html') {
                return this.serveStatic(res, path.join(__dirname, 'erd-demo.html'), 'text/html');
            }

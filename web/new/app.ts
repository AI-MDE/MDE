...
const rootArg: string | undefined = process.argv.find(a => a.startsWith('--root='));
const projectRoot: string = rootArg ? path.resolve(rootArg.replace('--root=', '')) : process.cwd();
//console.log(`[viewer] root Arg ${rootArg} argv: $process.agv ${process.argv.join(' ')}`);
//console.log(`[viewer]Project root: ${projectRoot}`);
const viewerUrl: string = 'http://localhost:4000/';
const viewerPort: number = 4000;

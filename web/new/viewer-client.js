...
/**
 * Opens the dashboard overview as a virtual document.
 */
function openDashboard() {
  closeMenu();
  const doc = { id: 'dashboard-overview', label: 'Dashboard', file: null, docType: 'dashboard' };
  _adHocDocs[doc.id] = doc;
  loadDoc(doc);
}
...
const VIRTUAL_FNS = {
  ...
  'dashboard':        (body) => renderDashboard(body),
};
...
const PATTERN_DOCTYPE = [
  ...
  { test: (doc) => doc.id === 'dashboard-overview',                          docType: 'dashboard' },
];
...
/**
 * Renders the AI-MDE dashboard overview from /api/dashboard.
 * @param {HTMLElement} body - The container to render into.
 */
async function renderDashboard(body) {
  body.innerHTML = `<div style="padding:40px;text-align:center;color:var(--muted)">Loading dashboard...</div>`;
  let data;
  try {
    data = await fetchJson('/api/dashboard');
  } catch (e) {
    body.innerHTML = `<p style="color:red">Failed to load dashboard data: ${escHtml(e.message)}</p>`;
    return;
  }

  const summary = data.summary || {};
  const phases = data.phases || [];
  const work = data.work || { done: [], next: [], attention: [] };
  ...
  const phaseCards = phases.map(p => `
    <div style="border:1px solid var(--border);border-radius:14px;padding:14px;background:#fff;box-shadow:0 2px 8px rgba(0,0,0,.04)">
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;margin-bottom:8px">
        <strong>${escHtml(p.name || '')}</strong>
        <span class="meta-badge">${escHtml(String(p.status || ''))}</span>
      </div>
      <div style="height:8px;background:#eef2f7;border-radius:999px;overflow:hidden;margin-bottom:10px">
        <div style="height:100%;width:${Number(p.percentComplete || 0)}%;background:#2563eb"></div>
      </div>
      <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;font-size:12px;color:var(--muted)">
        <div>Progress: <strong>${Number(p.percentComplete || 0)}%</strong></div>
        <div>Tasks: <strong>${Number(p.completedTaskCount || 0)}/${Number(p.taskCount || 0)}</strong></div>
        <div>Artifacts: <strong>${Number(p.artifactCount || 0)}</strong></div>
        <div>Blockers: <strong>${Number(p.blockerCount || 0)}</strong></div>
      </div>
    </div>`).join('');

  const itemList = (items, empty) => {
    if (!items || !items.length) return `<div style="color:var(--muted);font-size:13px">${escHtml(empty)}</div>`;
    return items.map(item => `
      <div style="padding:10px 0;border-top:1px solid #eef2f7">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:start">
          <div>
            <div style="font-weight:600">${escHtml(item.title || item.name || '')}</div>
            <div style="font-size:12px;color:var(--muted)">${escHtml(item.phase || item.type || item.severity || '')}${item.owner ? ' · ' + escHtml(item.owner) : ''}</div>
          </div>
          <span class="meta-badge">${escHtml(String(item.status || item.priority || ''))}</span>
        </div>
        ${item.detail ? `<div style="font-size:12px;color:#475569;margin-top:6px">${escHtml(item.detail)}</div>` : ''}
      </div>`).join('');
  };

  const recentActivity = (trace.recentActivity || []).slice(0, 8).map(a => `
    <div style="padding:8px 0;border-top:1px solid #eef2f7">
      <div style="font-weight:600">${escHtml(a.title || '')}</div>
      <div style="font-size:12px;color:var(--muted)">${escHtml(a.phase || a.actor || '')}${a.at ? ' · ' + escHtml(new Date(a.at).toLocaleString()) : ''}</div>
    </div>`).join('');

  body.innerHTML = `
    <div class="md-body"><h1>AI-MDE Dashboard</h1>
      <p>${escHtml(summary.description || 'Project delivery dashboard showing phase progress, work status, and next action.')}</p>
    </div>

    <div class="summary-grid" style="margin-bottom:18px">
      <div class="summary-card"><div class="lbl">Project</div><div class="val" style="font-size:22px">${escHtml(summary.projectName || '')}</div></div>
      <div class="summary-card"><div class="lbl">Current Phase</div><div class="val">${escHtml(summary.currentPhase || '-')}</div></div>
      <div class="summary-card"><div class="lbl">Completion</div><div class="val">${Number(summary.overallCompletion || 0)}%</div></div>
      <div class="summary-card"><div class="lbl">Status</div><div class="val">${escHtml(summary.status || '-')}</div></div>
      <div class="summary-card"><div class="lbl">Pending Approvals</div><div class="val">${Number(summary.pendingApprovals || 0)}</div></div>
      <div class="summary-card"><div class="lbl">Open Blockers</div><div class="val">${Number(summary.blockers || 0)}</div></div>
    </div>

    ${nextAction ? `
      <div style="border:1px solid #c7d2fe;background:#eef2ff;border-radius:16px;padding:16px 18px;margin-bottom:18px">
        <div style="font-size:12px;font-weight:700;letter-spacing:.04em;color:#4338ca;text-transform:uppercase;margin-bottom:6px">Recommended Next Action</div>
        <div style="font-size:20px;font-weight:700;color:#111827;margin-bottom:6px">${escHtml(nextAction.title || '')}</div>
        <div style="font-size:13px;color:#374151">${escHtml(nextAction.why || '')}</div>
        <div style="font-size:12px;color:#6b7280;margin-top:6px">Unlocks: ${escHtml(nextAction.unlocks || '-')}</div>
      </div>` : ''}

    <div style="margin-bottom:10px;font-size:16px;font-weight:700;color:#111827">Phases</div>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin-bottom:22px">${phaseCards}</div>

    <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;margin-bottom:22px">
      <div style="border:1px solid var(--border);border-radius:16px;padding:14px;background:#fff"><div style="font-size:14px;font-weight:700;margin-bottom:8px">Done</div>${itemList(work.done, 'Nothing completed yet.')}</div>
      <div style="border:1px solid var(--border);border-radius:16px;padding:14px;background:#fff"><div style="font-size:14px;font-weight:700;margin-bottom:8px">Next</div>${itemList(work.next, 'No next tasks found.')}</div>
      <div style="border:1px solid var(--border);border-radius:16px;padding:14px;background:#fff"><div style="font-size:14px;font-weight:700;margin-bottom:8px">Attention</div>${itemList(work.attention, 'No urgent issues.')}</div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1.2fr .8fr;gap:16px">
      <div style="border:1px solid var(--border);border-radius:16px;padding:14px;background:#fff">
        ...
        ${recentActivity || `<div style="color:var(--muted);font-size:13px">No recent activity.</div>`}
      </div>
      <div style="display:grid;gap:16px">
        <div style="border:1px solid var(--border);border-radius:16px;padding:14px;background:#fff"><div style="font-size:14px;font-weight:700;margin-bottom:8px">Blockers</div>${itemList(blockers.slice(0,6), 'No blockers.')}</div>
        <div style="border:1px solid var(--border);border-radius:16px;padding:14px;background:#fff"><div style="font-size:14px;font-weight:700;margin-bottom:8px">Decisions</div>${itemList(decisions.slice(0,6), 'No pending decisions.')}</div>
      </div>
    </div>`;
}

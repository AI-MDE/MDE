#!/usr/bin/env node
/**
 * Purpose: Validates project file/folder layout against expected MDE app structure conventions.
 */
'use strict';
 ...
 *
 * Usage:
 *   node scripts/validate-app-structure.js --project=<path>
 */
...
function loadJson(file, label) {
  try {
    ...
    err(`${label}: failed to parse â€” ${e.message}`);
    return null;
  }
}
...
// â”€â”€ 1. Configuration folders exist â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
console.log('â”€â”€ 1. Configuration folder paths exist â”€â”€');
const FOLDER_PATHS = [
...
for (const dotPath of FOLDER_PATHS) {
  ...
  if (fs.existsSync(abs)) ok(`config.${dotPath} â†’ ${val}`);
  else warn(`config.${dotPath} folder missing â€” ${val}`);
}

// â”€â”€ 2. Required artifacts per phase â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
console.log('\nâ”€â”€ 2. Required artifacts per current phase â”€â”€');
...
for (const phase of phasesToCheck) {
  for (const { label, path: relPath } of (REQUIRED_BY_PHASE[phase] || [])) {
    ...
    if (fs.existsSync(abs)) ok(`${phase}/${label} â€” ${relPath}`);
    else err(`${phase}/${label} missing â€” ${relPath}`);
  }
}

// â”€â”€ 3. Naming conventions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
console.log('\nâ”€â”€ 3. Naming conventions â”€â”€');
...
if (entitiesDir && fs.existsSync(entitiesDir)) {
  ...
  badEnt.forEach(f => err(`entity file "${f}" violates naming convention â€” expected ent-{kebab-name}.json`));
  if (!badEnt.length) ok(`${entityFiles.length} entity file(s) follow ent-*.json convention`);
} else {
  warn(`entities folder not found â€” ${cfg('design.entities')}`);
}
...
if (modulesDir && fs.existsSync(modulesDir)) {
  ...
  badMod.forEach(({ file, dir }) =>
    err(`module spec "${dir}/${file}" violates naming convention â€” expected module-{kebab-name}.json`)
  );
  ...
  warn(`modules folder not found â€” ${cfg('design.modules')}`);
}
...
if (useCasesDir && fs.existsSync(useCasesDir)) {
  ...
  badUc.forEach(f => err(`use-case file "${f}" violates naming convention â€” expected uc-{number}-{name}.md`));
  ...
  warn(`use-cases folder not found â€” ${cfg('ba.useCases')}`);
}

// â”€â”€ 4. Entity files: valid JSON with required fields â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
console.log('\nâ”€â”€ 4. Entity file structure â”€â”€');
...
for (const f of entityFiles) {
  ...
  if (missing.length) err(`${f}: missing required fields â€” ${missing.join(', ')}`);
  else ok(`${f}: id="${data.id}", name="${data.name}"`);
}

// â”€â”€ 5. Module catalog: every module has a spec file â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
console.log('\nâ”€â”€ 5. Module catalog â†’ spec file coverage â”€â”€');
...
const allModuleSpecFiles = new Map(); // basename â†’ abs path
if (modulesDir && fs.existsSync(modulesDir)) {
  (function walk(dir) {
...
const specFileById = new Map(); // moduleId â†’ abs path
...
if (catalog) {
  ...
  for (const mod of modules) {
    ...
    if (foundByName) {
      ok(`${modId} â†’ ${rel(foundByName)}`);
    } else {
      // Fallback: match by moduleId inside the spec file
      const foundById = specFileById.get(modId);
      if (foundById) {
        warn(`${modId} (${modName}): spec file name mismatch â€” found by ID at ${rel(foundById)}`);
      } else {
  ...
  warn(`module-catalog.json not found or invalid â€” skipping spec coverage check`);
}

// â”€â”€ 6. Use-case files have a title â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
console.log('\nâ”€â”€ 6. Use-case file titles â”€â”€');
...
// â”€â”€ 7. Module catalog entity IDs â†’ entity files exist â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
console.log('\nâ”€â”€ 7. Module catalog entity IDs â†’ entity files â”€â”€');
// Build entity ID â†’ file map from entity files
...
if (catalog) {
  ...
  for (const mod of modules) {
    ...
    for (const eid of entityIds) {
      if (entityIdToFile[eid]) ok(`${mod.moduleId || mod.id}: entity ${eid} â†’ ${entityIdToFile[eid]}`);
      else err(`${mod.moduleId || mod.id}: entity "${eid}" not found in any ent-*.json file`);
    }
  }
}

// â”€â”€ 8. Orphan entity files (entity ID not referenced by any module) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
console.log('\nâ”€â”€ 8. Orphan entity files â”€â”€');
...
// â”€â”€ 9. LDM entities cross-check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
console.log('\nâ”€â”€ 9. LDM entities vs entity files â”€â”€');
...
if (ldmFile && fs.existsSync(ldmFile)) {
  ...
  if (ldm) {
    ...
    // Convert PascalCase or spaces to kebab: "LeaveType" â†’ "leave-type"
    ...
    for (const name of ldmEntities) {
      ...
      if (entNames.has(kebab)) ok(`LDM entity "${name}" â†’ ent-${kebab}.json`);
  ...
  warn(`logical-data-model.json not found â€” ${ldmFile ? rel(ldmFile) : 'path not configured'}`);
}
